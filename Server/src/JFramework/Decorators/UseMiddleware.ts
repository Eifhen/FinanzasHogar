 ...
 */
export default function UseMiddleware(middlewareFactory: MiddlewareFactory | MiddlewareFactory[]) {
    ...
    const wrappedMiddlewares = factories.map((factory) => {
      return (req: ApplicationRequest, res: Response, next: NextFunction) => {
        ...
        if (isApplicationMiddleware(factory)) {
          ...
          resolvedMiddleware = container.build(factory as FunctionReturning<ApplicationRequestHandler>);
